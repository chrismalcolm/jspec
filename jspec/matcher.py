...
def match_object_traverse(loc, term, element, term_len, element_len):
    ...
    if len(element) == 0:
        if term.satisfied_captures():
            return GoodMatch()
        return BadMatch(loc, "exhausted JSON object, failed to match the following JSPEC pairs: [%s]" % ", ".join(sorted([str(p) for p in spec]))).with_capture_metadata(term_len, element_len)

    if term.exhausted_captures():
        return BadMatch(loc, "exhausted JSPEC object, failed to match the following JSON pairs: [%s]" % ", ".join([(json.dumps(k)+": "+json.dumps(v)) for k,v in sorted(element.items())])).with_capture_metadata(term_len, element_len)
    ...
    for spec_pair in spec:
        for element_pair in element.items():
            if isinstance(spec_pair, JSPECObjectPair):
                result = match_object_pair(loc, spec_pair, element_pair).with_capture_metadata(term_len, element_len)
                if bool(result):
                    new_term = JSPECObject(set(p for p in spec if p != spec_pair))
                    new_element = dict(p for p in element.items() if p != element_pair)
                    return match_object_traverse(loc, new_term, new_element, term_len+1, element_len+1)
...
def match_object_capture_group(loc, capture, element_pair, term_len, element_len):
    ...
    return None, BadMatch(loc, "failed object capture, '%s: %s' failed to match '%s'" % (json.dumps(element_pair[0]), json.dumps(element_pair[1]), capture)).with_capture_metadata(term_len, element_len)
...
def match_element(loc, term, element):
    ...
    Args:
        term (JSPECTerm): The JSPEC element.
        element (element): The python native object.
